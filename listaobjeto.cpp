...
void ListaObjeto::Delete(NodoObjeto *raiz){
    if(raiz==NULL){
        return;
    }
...
void ListaObjeto::insertar(int id,string name,char letter, string color,int Cx,int Cy){
    this->cabeza=agregar(this->cabeza,id,name, letter,  color, Cx, Cy);
}

NodoObjeto* ListaObjeto::agregar(NodoObjeto* cabeza, int id,string name,char letter, string color,int Cx,int Cy){
    if(cabeza==NULL){
        cabeza= new NodoObjeto (id, name, letter,  color, Cx,Cy);
    }else{
        if(id<cabeza->id){
...
void ListaObjeto::generar(NodoObjeto* padre,NodoObjeto* actual, bool ac){
    if(actual!=NULL){
       if(ac){
